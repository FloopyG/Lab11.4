...
void phoneSearch(const char* fName);
...
void phoneSearch(const char* fName) {
    ...
    while (f.read((char*)&note, sizeof(Note))) {
        if (strcmp(note.phoneNum, userStr) == 0) {
            cout << endl;
            cout << n++ << "Дані про людину, з відповідним номером телефону" << endl << endl;
            cout << "Прізвище та ім'я: " << note.fullName << endl;
            cout << "Номер телефону: " << note.phoneNum << endl;
            cout << "Дата народження: " << setfill('0') << setw(2) << note.birthDate[0] << "." << setfill('0') << setw(2) << note.birthDate[1] << "." << setfill(' ') << setw(4) << note.birthDate[2] << endl;
            cout << endl;
